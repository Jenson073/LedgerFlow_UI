  ...
  <div class="flex flex-col gap-5 w-full">
    ...
    <div class="relative w-full overflow-x-auto rounded-md bg-white/[0.01] border border-border">
      <div v-if="loading" class="absolute inset-0 bg-slate-950/65 backdrop-blur-sm flex flex-col items-center justify-center gap-2 z-10">
        ...
        <p class="text-sm font-medium text-text-base">Updating receipt vault...</p>
      </div>

      <table class="w-full border-collapse text-left text-sm">
        <thead>
          <tr>
            <th class="px-4 py-3.5 font-semibold text-text-muted border-b border-border bg-white/[0.02]">Receipt #</th>
            <th class="px-4 py-3.5 font-semibold text-text-muted border-b border-border bg-white/[0.02]">Merchant</th>
        ...
        <tbody>
          <tr v-if="filteredReceipts.length === 0">
            <td colspan="6" class="text-center p-12 text-text-muted">No receipts found.</td>
          </tr>
          <tr v-for="receipt in filteredReceipts" :key="receipt.id" class="hover:bg-white/[0.01] transition-colors">
            <td class="p-4 border-b border-white/[0.04] text-text-base font-semibold">{{ receipt.receiptNumber }}</td>
            <td class="p-4 border-b border-white/[0.04] text-text-base">{{ receipt.merchantName }}</td>
            <td class="p-4 border-b border-white/[0.04] text-text-base">{{ formatDate(receipt.date) }}</td>
            <td class="p-4 border-b border-white/[0.04] text-text-base">
              <span class="inline-flex px-2 py-0.5 rounded-sm bg-accent-subtle text-accent-alt text-[0.775rem] font-medium border border-accent/15">{{ receipt.category }}</span>
            </td>
            <td class="p-4 border-b border-white/[0.04] text-text-base font-semibold tabular-nums">{{ formatCurrency(receipt.amount) }}</td>
            <td class="p-4 border-b border-white/[0.04] text-text-muted max-w-[15rem] truncate" :title="receipt.notes">{{ receipt.notes || '—' }}</td>
          </tr>
        </tbody>
      </table>
    </div>
